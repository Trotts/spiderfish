...
do
    ...
    echo -e "\tCrawling. This might take a while..."
    
    # cd to directory where local spider is stored and initiate search
    cd $swd/fishbase
    # Pipes output to /dev/null to stop warnings/logging to terminal. Remove &> /dev/null for debugging.
    scrapy crawl fish -a family=$family -o $family.json &> /dev/null

    # remove SHA1 tagged files/keep only the renamed ones
    find ./output/full -type f -name "????????????????????*.*" -exec rm -rf {} \;

    # move all downloaded images and output JSON file to results folder
    mv ./output/full/*.jpg $2/$family
    rm ./output/full/*.gif
    mv $family.json $2/$family

    # initiate image classification
    echo -e "\tWorking on image classification..."
